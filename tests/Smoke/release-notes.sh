...
grep -Fq 'php scripts/release-asset-manifest.php create dist "$GITHUB_REF_NAME" "$GITHUB_SHA"' "$workflow"
grep -Fq 'run: sh scripts/verify-published-release.sh "$GITHUB_REF_NAME"' "$workflow"
